"""Minimal graph-refinement Stage 2 model built on top of joint_residual."""
...
from ..components import CTRGraphBlock, MLP, build_smpl_body23_graph


@dataclass(slots=True)
class Stage2JointGraphRefinerConfig:
    """Configuration for the graph-refiner Stage 2 fusion model."""

    name: str = "stage2_joint_graph_refiner"
...
class Stage2JointGraphRefinerModel(nn.Module):
    """Stable joint-residual fusion with one extra graph-based pose residual."""

    def __init__(self, config: Stage2JointGraphRefinerConfig) -> None:
        super().__init__()
        self.config = config
        ...
        adjacency = build_smpl_body23_graph()
        self.pose_graph_blocks = nn.ModuleList(
            [
                CTRGraphBlock(
                    config.joint_latent_dim,
                    config.joint_latent_dim,
                    adjacency,
                    adaptive=config.adaptive_graph,
                )
                for _ in range(config.graph_layers)
            ]
        )
        self.pose_graph_head = MLP(
            input_dim=config.joint_latent_dim,
            hidden_dim=config.hidden_dim,
            output_dim=6,
            num_layers=config.refinement_layers,
    ...
    def forward(self, views_input: torch.Tensor) -> dict[str, torch.Tensor]:
        if views_input.ndim != 3:
            raise ValueError(
                "Stage2JointGraphRefinerModel expects views_input with shape "
        ...
        for graph_block in self.pose_graph_blocks:
            graph_features = graph_block(graph_features)
        graph_features = graph_features.squeeze(-2).transpose(1, 2)
        graph_pose_delta = self.pose_graph_head(
            graph_features.reshape(batch_size * self.config.num_joints, -1)
        ).reshape(batch_size, self.config.num_joints, 6)
        graph_pose_delta = self.config.graph_delta_scale * torch.tanh(graph_pose_delta)
        pred_pose_6d = init_pose_6d + local_pose_delta + graph_pose_delta
